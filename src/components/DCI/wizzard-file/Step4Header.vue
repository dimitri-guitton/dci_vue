        ...
        <div class="col-md-4">
            <label class="form-check form-switch form-check-custom">
                <Field
                    ...
                    v-model="bonus"
                />
                <span class="form-check-label fw-bold text-gray-400">Désactiver les primes</span>
            </label>
        </div>
        <div class="col-md-4">
            <label class="form-check form-switch form-check-custom">
                <Field
                    ...
                    v-model="ceeBonus"
                />
                <span class="form-check-label fw-bold text-gray-400">Désactiver la prime CEE</span>
            </label>
        </div>
        <div class="col-md-4">
            <label class="form-check form-switch form-check-custom">
                <Field
                    ...
                    :value="true"
                    v-model="maPrimeRenovBonus"
                />
                <span class="form-check-label fw-bold text-gray-400">Désactiver MaPrimeRenov</span>
            </label>
        </div>
    </div>
    <el-divider class="mb-10"></el-divider>
...
export default defineComponent( {
                                    ...
                                    setup( props, ctx ) {
                                        ...
                                        watch( bonus, newValue => {
                                            ctx.emit( 'bonusAreUpdated',
                                                      {
                                                          bonus:             newValue ?? false,
                                                          ceeBonus:          ceeBonus.value ?? false,
                                                          maPrimeRenovBonus: maPrimeRenovBonus.value ?? false,
                                                      } );
                                        } );

                                        watch( ceeBonus, newValue => {
                                            ctx.emit( 'bonusAreUpdated',
                                                      {
                                                          bonus:             bonus.value ?? false,
                                                          ceeBonus:          newValue ?? false,
                                                          maPrimeRenovBonus: maPrimeRenovBonus.value ?? false,
                                                      } );
                                        } );

                                        watch( maPrimeRenovBonus, newValue => {
                                            ctx.emit( 'bonusAreUpdated',
                                                      {
                                                          bonus:             bonus.value ?? false,
                                                          ceeBonus:          ceeBonus.value ?? false,
                                                          maPrimeRenovBonus: newValue ?? false,
                                                      } );
                                        } );
